...
# Image.gz是你编译出来的产物，丢到这里，然后后面的kernel.img是这里的打包产物。中间几个参数按照官方来没啥问题，注意中间时间os_patch_level需要参考你原来内核的系统的安卓安全补丁版本，最后编译完了只会精确到月份，内核的这个时间如果比系统的安全补丁版本要早，就会造成开不开机，等于能开，晚于我也没试过。
